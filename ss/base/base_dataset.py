...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        wave_augments, audio_wave = self.process_wave(audio_wave)
        return {
            "audio": audio_wave,
    ...
    def process_wave(self, audio_tensor_wave: Tensor):
        with torch.no_grad():
            ...
            if self.augs is not None:
                audio_tensor_wave = self.augs(audio_tensor_wave)
                augments = repr(self.augs)
            return augments, audio_tensor_wave
    ...
    def _filter_records_from_dataset(
        ...
        if max_text_length is not None:
            exceeds_text_length = (
                    np.array(
                        [len(re.sub(r"[^a-z ]", "", el["text"].lower())) for el in index]
                    )
                    >= max_text_length
            )

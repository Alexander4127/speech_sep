...
class Trainer(BaseTrainer):
    ...
    def move_batch_to_device(batch, device: torch.device):
        ...
        """
        for tensor_for_gpu in ["mix", "target", "ref", "speaker_id"]:
    ...
    def _log_audio(self, **kwargs):
        if self.writer is None:
            return
        idx = np.random.choice(np.arange(len(kwargs["path"])))
        for t_name in filter(lambda s: s.endswith("_path"), kwargs.keys()):
            audio = kwargs[t_name.split('_')[0]]
            audio_path = kwargs[t_name]
            t_info = torchaudio.info(audio_path[idx])
            self.writer.add_audio(f"{t_name} audio", audio[idx], t_info.sample_rate)

        sr = self.config["preprocessing"]["sr"]
        for dur in ["short", "middle", "long"]:
            self.writer.add_audio(f"pred audio ({dur})", kwargs[dur][idx], sr)
